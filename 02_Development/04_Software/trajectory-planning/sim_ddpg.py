...
from trajectoryplan.trajplan import policy, get_actor, get_critic
...
version = 'v2'
weights = 'vehicle_env_continuous_v2_ay_nolim_obs0'
...
if __name__ == "__main__":
    ...
    while True:
        ...
        action, action_raw, action_noise = policy(tf_prev_state, 0, actor_model, lower_bound, upper_bound)
    ...
    if ay_plot:
        ...
        ax3.plot(x_obs_record, 'b')
        ax3.plot(y_obs_record, 'r')
        ax3.set_title("x, y obstacle [-]")
        ax3.grid(color='gray', linestyle='-', linewidth=1)
        ax4.plot(time, steer_record)
        ax4.set_title("delta [-]")
        ax4.set_yticks(np.arange(-1, 1.2, step=0.5))
        ax4.grid(color='gray', linestyle='-', linewidth=1)
        ax5.set_title("Lateral acceleration [m/s2]")
        ax5.plot(time, a_lat_record)
        ax5.set_yticks(np.arange(-10, 11, step=5))
        ax5.grid(color='gray', linestyle='-', linewidth=1)
        ax6.set_title("Episodic Cumulative Reward is ==> {}".format(episodic_reward))
        ax6.set_xlabel("Time [s]")
        ax6.plot(time, reward_record)
