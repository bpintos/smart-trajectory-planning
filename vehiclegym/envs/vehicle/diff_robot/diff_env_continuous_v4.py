...
class VehicleTfmEnv(gym.Env):
    ...
    def _reward(self):
        ...
        # Get state
        state_lasers = self.state[:self.number_laser_rays]
        vel_norm = self.state[-2]
        # vel_ang = self.state[-1]
        diff_heading = 0
        ...
        if not(laser_sick_error):
            ...
            # reward_obs = np.clip(-np.exp(-25*(min_state_laser*self.laser_range-self.min_distance2robot)),-1,0)
            reward_obs = np.clip(-np.exp(-20*(min_state_laser*self.laser_range-0.3)),-1,0)
            # reward_obs = np.clip((min_state_laser*self.laser_range - min_distance)/(min_distance - self.min_distance2robot), -1, 0)
            # reward_obs = np.clip((min_state_laser*self.laser_range - min_distance)/(min_distance - 0.3), -1, 0)
        else:
            reward_obs = 0
        ...
        # Penalization if angular acceleration is too high and if not travelling at maximum velocity
        if all(IMU_error == False for IMU_error in IMUs_error):
            reward_accel = np.clip((0.0005 - abs(accel_ang))/0.0020, -1, 0)
            reward_vel = vel_norm - 1
            # reward_vel = -np.exp(-4*vel_norm)
        
        # Print reward values
        print('Min laser distance: ', min_state_laser*self.laser_range)
        print('reward_obs:', reward_obs)
        print('reward_vel:', reward_vel)
        # print('reward_accel:', reward_accel)
        # print('reward_target_heading:', reward_target_heading)

        reward = 5*reward_obs + 1*reward_vel + 0*reward_accel + 0*reward_target_heading
    ...
    def _getState(self):
        ...
        # state = front_norm_lasers + [vel_norm_x] + [vel_norm_y] + [vel_ang] + [distance2target]
        # state = front_norm_lasers + [vel_norm_ang]
        # state = list(laser_distances) + [vel_norm_ang] + [distance2target]
        # state = list(laser_distances) + [vel_norm_lin]
        state = list(laser_distances) + [vel_norm_lin] + [vel_norm_ang]
        # state = list(laser_distances) + [heading/np.pi]
