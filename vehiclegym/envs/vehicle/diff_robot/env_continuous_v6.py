...
class VehicleTfmEnv(gym.Env):
    ...
    def _reward(self):
        ...
        diff_heading_corrected = self.state[self.number_laser_rays + 4]
        print('distance2target: ', distance2target)
        print('x_goal: ', self.goal[0], 'y_goal: ', self.goal[1])
        print('diff_heading_corrected: ', diff_heading_corrected)
        ...
        if distance2target >= 0.25:
            ...
            vel_target = 0
        if all(IMU_error == False for IMU_error in IMUs_error):
            reward_task_1 = np.clip(1-(vel_target - vel)/vel_target, 0, 1)
        else:
            reward_task_1 = 0
        
        # Task 2: Penalization if robot does not face the destination (Destination requirements)
        if all(position_error == False for position_error in positions_error):
            reward_task_2 = -abs(diff_heading_corrected)
        else:
            reward_task_2 = 0
        
        reward_task = reward_task_1 + reward_task_2
        ...
        print ('Reward task 1:', reward_task_1)
        print ('Reward task 2:', reward_task_2)
    ...
    def _getState(self):
        ...
        state = list(laser_distances) + [vel_norm_lin] + [vel_norm_ang] + [accel_ang_norm] + [distance2target] + [diff_heading_corrected]
