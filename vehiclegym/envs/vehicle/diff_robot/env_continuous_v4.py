...
Two actions (linear and angular velocities)
State: laser measurements + linear velocity + angular velocity + angular acceleration + distance to destination
Safety, legal, comfort and task (energy) requirements
Stop strategy at destination
...
class VehicleTfmEnv(gym.Env):
    ...
    def __init__(self, number_proximity, number_laser, number_IMU, number_GPS, number_laser_rays,\
        ...
        high = np.append(high, [1, 1, 1, 1, 1])
        low = np.zeros(self.number_laser_rays, dtype = np.float32)
        low = np.append(low, [0, -1, -1, -1, 0])
    ...
    def step(self, action):
        ...
        # Unnormalize action
        actionLinearVelocity = action[0]*self.robotMaxLinearVelocity
        actionAngularVelocity = action[1]*self.robotMaxAngularVelocity
    ...
    def _isDone(self):
        ...
        # accel_lin = self.sensors[20]
        # heading = self.sensors[21]
        pos_x = self.sensors[22]
        pos_y = self.sensors[23]
        # laser_sick = self.sensors[24]

        vel = np.sqrt(vel_x**2 + vel_y**2)

        IMUs_error = self.sensors_error[16:21]
        positions_error = self.sensors_error[21:24]
    ...
    def _reward(self):
        ...
        accel_lin_norm = self.state[self.number_laser_rays + 3]
        distance2target = self.state[self.number_laser_rays + 4]
        
        # Local variables
        # vel = np.sqrt(vel_x**2 + vel_y**2)
        vel = vel_norm_lin*self.robotMaxLinearVelocity
        accel_ang = accel_ang_norm*5
        accel_lin = accel_lin_norm*2
        ...
        if all(IMU_error == False for IMU_error in IMUs_error):
            reward_comfort_ang = np.clip((accel_max_ang - abs(accel_ang))/2, -1, 0)
            reward_comfort_lin = np.clip((accel_max_lin - abs(accel_lin))/1, -1, 0)
            reward_comfort = (reward_comfort_ang + reward_comfort_lin)/2
        else:
            reward_comfort = 0
        
        # Penalization if robot exceeds maximum velocity (Legal requirements)
        if (pos_x >= self.goal[0]-0.5 and pos_y >= self.goal[1]-0.5\
            and pos_x <= self.goal[0]+0.5 and pos_y <= self.goal[1]+0.5)\
                or self.stop_flag:
                    vel_max = 0
                    self.stop_flag = True
        ...
        print ('Linear velocity:', vel, ', vel_x:', vel_x, ', vel_y:', vel_y, ', vel_ang', vel_ang)
        print ('accel_ang:', accel_ang, ', accel_lin:', accel_lin)
        print ('Target linear velocity:', vel_target)
        # print ('vel_max:', vel_max)
        print ('Reward safety:', reward_safety)
        print ('Reward comfort ang:', reward_comfort_ang, ', Reward comfort lin:', reward_comfort_lin)
        print ('Reward legal:', reward_legal)
        print ('Reward task:', reward_task)
        print ('heading', heading*180/np.pi, ', pos_x', pos_x, ', pos_y', pos_y)
        #print ('pos_x', pos_x)
        
        
        # Final reward
        reward = k_safety*reward_safety + k_legal*reward_legal + k_comfort*reward_comfort + k_task*reward_task
    ...
    def _getSensors(self):
        ...
        if raw_data[0] == sim.simx_return_ok:
            ...
            vel_linear = np.sqrt(vel_x**2 + vel_y**2)
            accel_linear = (vel_linear-self.vel_linear_old)/dt
            ...
            sensors_error.append(False)
            sensors.append(accel_linear)
            sensors_error.append(False)
            self.vel_angular_old = vel_angular
            self.vel_linear_old = vel_linear
    ...
    def _getState(self):
        ...
        accel_ang_norm = np.clip(accel_ang/5, -1, 1)
        accel_lin_norm = np.clip(accel_lin/2, -1, 1)
        ...
        state = list(laser_distances) + [vel_norm_lin] + [vel_norm_ang] + [accel_ang_norm] + [accel_lin_norm] + [distance2target]
